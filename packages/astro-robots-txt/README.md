...
| `sitemap` | `Boolean / String` / `String[]` |              `true`              | Resulting output in a _robots.txt_ will be `Sitemap: your-site-url/sitemap.xml`.<br/>If `sitemap: false` - no `Sitemap` line in the output.<br/>You could use for the `sitemap` a valid **http** url string or array of **http** url strings.<br/>Empty strings are not allowed. |
