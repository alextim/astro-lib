...
| `sitemap` | `Boolean / String` / `String[]` |              `true`              | Resulting output in a _robots.txt_ will be `Sitemap: your-site-url/sitemap.xml`.<br/>If `sitemap: false` - no `Sitemap` line in the output.<br/>When `sitemap` is `String` or `String[]` the values have to be a valid **http** url. Empty strings are not allowed. |
